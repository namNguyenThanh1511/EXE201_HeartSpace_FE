...
function normalizeRole(role: unknown): "consultant" | "other" {
  const r = String(role ?? "").trim().toLowerCase();
  return r === "consultant" ? "consultant" : "other";
}

/* =================================================================
   ===============   GIAO DIỆN DÀNH CHO CONSULTANT   ===============
   ================================================================= */
...
/* Row cho CONSULTANT: dùng thêm API appointment-id để lấy tên + thời gian chuẩn */
function ConsultantRow({ ev }: { ev: AppointmentDetailResponse }) {
...
function ConsultantAppointmentsView() {
          ...
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                ...
                {!isLoading && !isError && items.map((ev) => <ConsultantRow key={ev.id} ev={ev} />)}
              </TableBody>
            </Table>
          </div>
        </CardContent>
...
/* =================================================================
   ==============   GIAO DIỆN DÀNH CHO NGƯỜI DÙNG KHÁC   ===========
   ================================================================= */
...
function formatAppointmentStatus(status?: string) {
  switch (status) {
    ...
    case "PendingPayment":
      return { text: "Chờ thanh toán", variant: "outline" as const };
    case "Paid":
    ...
    default:
      return { text: "Chưa xác định", variant: "secondary" as const };
  }
}
      ...
      <TableCell>
        <div className="flex items-center gap-3">
          <Avatar className="h-9 w-9">
            ...
            <AvatarFallback>
              {isLoadingConsultant ? <Loader2 className="h-4 w-4 animate-spin" /> : getInitials(consultantInfo.name)}
            </AvatarFallback>
          </Avatar>
          <div className="space-y-0.5">
          ...
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                ...
                {!isLoading && !isError && items.map((ev) => <ClientRow key={ev.id} ev={ev} />)}
              </TableBody>
            </Table>
          </div>
        </CardContent>
